...
Additionally, you will need to create two separate .env files: one for the client and one for the server. Both need to be created inside the root folder of client/server and be structured as follows:
/client/.env
```
...
To start the server as well as database make sure that you start the docker daemon on your computer. This allows to start the dockerized containers by running the command `docker-compose up`. In case that you want to force a rebuild of the images before starting the container, run the command `docker-compose up --build`. That is necessary when changes have been made to the Dockerfile or source code that affects the image.
...
PORT=3003
```
All the CRUD (create, read, update, delete) operations are implemented, see Appendix 2.
...
<img src="design-01-landing_page.png" alt="Start Page" width="200">
